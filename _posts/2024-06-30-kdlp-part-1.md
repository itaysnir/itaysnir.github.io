...
1. `softirq`s - Still runs in an interrupt context (can't block), starts immediately after the ISR (can't be scheduled), but this time with interrupts enabled, thus is preemptive.

2. Another type are `tasklet`s, which are similar to softirq, but can be deferred(hence, now schedule-able). 

3. `workqueue`s are even weaker version, now also having a process context and can sleep. Used for more extensive processing, for example, DMA operations, locks, etc. 
